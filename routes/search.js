...
router.get('/users', authMiddleware, (req, res) => {
  ...
  const query = `
    SELECT 
      u.id, u.username, u.fullname, u.profile_picture, u.bio,
    ...
    WHERE (u.username LIKE ? OR u.fullname LIKE ? OR u.bio LIKE ?)
      AND u.id != ?
      AND b2.id IS NULL
    ORDER BY 
      CASE 
  ...
  db.all(query, [currentUserId, currentUserId, searchTerm, searchTerm, searchTerm, currentUserId, exactTerm, exactTerm], (err, users) => {
    if (err) {
